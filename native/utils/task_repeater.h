...
// call function every interval_ms in separated thread.
class task_repeater
{
...
public:
	...
	task_repeater(const std::string name, std::function<void()> func, int interval_ms)
	{
		thread = std::thread([=] {
			...
			try
			{
				...
				{
					func();
					if (sleep.interrupted(interval_ms))
						break;
				}
			}
	...
	static void test()
	{
		...
		auto test1 = [&] {
			struct FOBJ
			{
				int seed = 0;
				elapsed tick;
				const char *label = "task_repeater label arg";
				void check()
				{
			...
			task_repeater m("test1", handler, 1000);
		...
		auto test2 = [] {
			...
			task_repeater m("test2", handler, 1000);
		...
		auto test3 = [] {
			...
			task_repeater m("test3", handler, 1000);
		...
		auto test4 = [] {
			...
			task_repeater m("test4", handler, 1000);
